/**
 * Cloudflare API helpers for the vozka CLI — token verification, account resolution, and zone lookup.
...
/**
 * List the account's zones (`GET /zones?account.id=<id>`), used to SUGGEST a vozka domain
 * (`vozka.<zone>`). Best-effort: returns [] on any error (the token may lack Zone:Read) — the caller
