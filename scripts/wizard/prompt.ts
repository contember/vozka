/**
 * Interactive prompts for the bootstrap wizard, over `node:readline/promises` on the real TTY.
 *
 ...
 * supplied and accepted). Used for values the wizard cannot proceed without.
 */
 ...
 * source (never the value) — so an operator can pre-set e.g. `CLOUDFLARE_API_TOKEN=… bun run …` and
...
/**
 * Run an async action that VALIDATES input (verify a token, mint a key, resolve a path). On a thrown
 * error, show the message and re-ask — so a typo'd or rejected value re-prompts in place instead of
 * crashing the whole wizard and losing every value already entered. Declining the retry re-throws, so
 * the caller's top-level handler still aborts cleanly. The `action` must itself re-read its inputs (do
 * the `secret()`/`text()` prompt INSIDE it) so a retry actually asks again.
 */
